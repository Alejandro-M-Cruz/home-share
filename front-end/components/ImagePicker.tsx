...
>(({
  ...
  return (
    <View ref={ref} {...props}>
      ...
      <Button onPress={pickImages} className="mx-auto flex flex-row space-x-2">
        <MaterialCommunityIcons name="image-multiple" size={20} className="text-primary-foreground" />
      ...
      {images?.length > 0 && (
        <>
          <FlatList
            ...
            renderItem={({ item: image }) => (
              <View className="border-4 border-background shadow-md rounded-lg relative m-4 p-0">
                ...
                />
                <Button
                  onPress={() => onImagesChange(images.filter(i => i.uri !== image.uri))}
                  variant="outline"
                  size="icon"
          ...
          >
            <MaterialCommunityIcons name="delete-forever" className="text-destructive-foreground" size={20} />
