      ...
      <CardFooter className="flex-grow flex flex-col py-3 mx-0 px-0">
        <View className="flex-grow grid grid-cols-6 sm:grid-cols-12 gap-y-3 gap-x-3 max-sm:mx-2 px-5">
          ...
          <View className="col-span-12 xl:col-span-4 flex flex-row flex-wrap items-center justify-center">
            <Text className="font-semibold text-base text-center ">${rentalListing.monthlyRent.toFixed(2)}</Text>
        ...
        {variant === 'my_rental_listing' && (
          <>
            ...
            <View className="flex flex-row justify-between w-full items-center space-x-2 px-3 relative">
              <Badge className={cn(rentalListing.status === 'active' ? 'bg-green-500' : 'bg-amber-500')}>
                <Text className="text-primary-foreground">{statusLabels[rentalListing.status]}</Text>
              </Badge>
              <Button onPress={handleStatusToggle} size="icon" variant="ghost" className="rounded-full absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
              ...
              <Button onPress={handleDelete} size="icon" variant="ghost" className="rounded-full web:hover:bg-destructive active:bg-destructive group">
                <MaterialCommunityIcons
